...
## Step 7: Create Custom IAM Policy (Production-Ready, Minimal Permissions)
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::YOUR-BUCKET-NAME/properties/*"
    },
    {
...
**Why this policy is secure for production:**
- **No `s3:PutObjectAcl`** - Prevents changing object permissions (public access is handled by bucket policy)
- **Restricted to `/properties/*` prefix** - Only allows access to a specific folder, not the entire bucket
- **Conditional ListBucket** - Can only list objects within the properties prefix
- **Sid identifiers** - Better for auditing and debugging

3. Click **"Next"**
4. **Policy name**: `RealEstateS3UploadPolicy`
5. **Description**: `Production-ready minimal S3 permissions for real estate image uploads`
